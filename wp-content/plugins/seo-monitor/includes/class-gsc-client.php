...
class SEOM_GSC_Client {
    ...
    /**
     * Fetch search appearance data for all pages.
     * GSC API doesn't allow combining searchAppearance with page dimensions,
     * so we make one call per appearance type, filtered to just pages with that feature.
     *
     * Returns: [url => [ 'FAQ_RICH_RESULT' => {clicks,impressions}, ... ]]
     *
     * @param int $days Lookback period
     * @return array|WP_Error
     */
    public function get_all_search_appearances($days = 28) {
        $end_date   = date('Y-m-d', strtotime('-3 days'));
        $start_date = date('Y-m-d', strtotime("-{$days} days"));

        // Correct GSC API searchAppearance filter values
        // Ref: https://searchsignals.com/search-appearance-mapping-for-gsc-api-codes
        $appearance_types = [
            'TPF_FAQ',            // FAQ rich results
            'TPF_HOWTO',          // How-to rich results
            'TPF_QA',             // Q&A rich results
            'RICHCARD',           // Rich cards / rich results
            'REVIEW_SNIPPET',     // Review stars in SERP
            'VIDEO',              // Video results
            'ORGANIC_SHOPPING',   // Product/shopping results
            'PAGE_EXPERIENCE',    // Good page experience
        ];

        $all_rows = [];

        foreach ($appearance_types as $type) {
            $start_row = 0;
            $batch_size = 5000;
            ...
            do {
                ...
                if (is_wp_error($result)) {
                    // Non-fatal per type — skip this appearance type
                    break;
                }
